...
<body>
    ...
    @php
        $user = Auth::user();
        $equipo = $user->teams->first(); // Obtenemos el equipo del usuario
    ...
    <main class="container">
        ...
        @else
            <h1 id="evento">Entrega del proyecto: {{ $evento ? $evento->title : 'Evento Desconocido' }}</h1>
            ...
            <div class="box">
                ...
                @if($equipo->project_file_path)
                    <div id="entregado">
                        ...
                        <p>Archivo actual: <a href="{{ route('equipos.descargar', $equipo->id) }}" target="_blank" style="color: blue; text-decoration: underline;">Descargar evidencia subida</a></p>
                        ...
                        @if($equipo->evaluations->count() > 0)
                            <div class="judge-feedback">
                                ...
                                @foreach($equipo->evaluations as $eval)
                                    ...
                                    <p><strong>Comentarios:</strong> {{ $eval->feedback ?? 'Sin comentarios adicionales.' }}</p>
                ...
                <div id="subida" class="{{ $equipo->project_file_path ? 'hidden' : '' }}">
                    ...
                    <form action="{{ route('equipos.subir_archivo') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        
                        <label class="file-label" style="display: block; margin-bottom: 20px;">
                            Seleccionar archivo (PDF, ZIP, RAR, DOCX - Max 10MB)
                            <input type="file" name="archivo_proyecto" id="archivo" accept=".pdf,.zip,.rar,.doc,.docx" required onchange="mostrarNombreArchivo()">
                        </label>

                        <p id="preview" class="preview-text" style="margin-bottom: 20px;">Ningún archivo seleccionado</p>

                        <button type="submit" class="btn enviar">Subir y Enviar</button>
                        
                        @if($equipo->project_file_path)
                            <button type="button" class="btn cancel" onclick="cancelarEdicion()" style="background-color: #666; margin-left: 10px;">Cancelar</button>
                        @endif
                    </form>
