...
<body>
    ...
    <section class="search-section" style="max-width: 1200px; margin: 20px auto; padding: 0 20px;">
        <form action="{{ route('dashboard.juez') }}" method="GET" style="display: flex; gap: 10px; align-items: center;">
            <div class="search-box" style="flex: 1; display: flex; gap: 10px;">
                <input type="text" 
                       name="search" 
                ...
                <button type="submit" 
                        class="btn-search"
                        style="padding: 12px 24px; background-color: #333; color: white; border: none; border-radius: 5px; cursor: pointer; font-weight: bold;">
                    Buscar
                </button>

                @if(request('search'))
                    <a href="{{ route('dashboard.juez') }}" 
                       style="padding: 12px 24px; background-color: #777; color: white; border: none; border-radius: 5px; text-decoration: none; font-weight: bold; display: flex; align-items: center;">
                        Limpiar
                    </a>
                @endif
            </div>
        ...
        <div class="events-grid">
            ...
            @else
                <div style="grid-column: 1 / -1; text-align: center; padding: 50px;">
                    @if(request('search'))
                        <h3>No se encontraron eventos con "{{ request('search') }}" 🔍</h3>
